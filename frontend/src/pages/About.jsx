...
export default function Home() {
  ...
  const fetchMeds = async () => {
    try {
      ...
      } else if (response.data && Array.isArray(response.data.medications)) {
      ...
      setErrorNotice(' Hindi makakonekta sa Backend/Database Server (localhost:5000). Siguraduhing nakatakbo ang Node.js backend.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const lowRefills = meds.filter((med) => (med.refillCount ?? med.refills) <= 2);
  ...
  return (
    <div className="page">
      ...
      {errorNotice && (
        <div style={{ background: '#fee2e2', border: '1px solid #f87171', color: '#991b1b', padding: '12px 16px', borderRadius: '8px', marginBottom: '16px', fontSize: '14px' }}>
          {errorNotice}
        </div>
      )}
              ...
              >
                ...
                <div className="medication-details">
                  ...
                  <div className="detail-box">
                    ...
                    <div
                      className={
                        (med.refillCount ?? med.refills ?? 0) <= 2
                          ? 'detail-value refill-low'
                          : 'detail-value refill-good'
                      }
                    >
                      🔄 {med.refillCount ?? med.refills ?? 0} remaining
                    </div>
                  </div>
                </div>
